...
public class TodoService {
	...
	private void validate(final TodoEntity entity) {
	 ...
	 public List<TodoEntity> retrieve(final String userId) {
	        return repository.findByUserId(userId);  // ')'를 삭제하여 오류 수정
	    }
	 ...
	 public List<TodoEntity>update(final TodoEntity entity) {
		 ...
		 //(2) 넘겨받은 엔티티 id 를 이용해 TodoEntity를 가져온다. 존재하지 않는 엔티티는 업데이트 할 수 없기 떄문이다.
	 ...
	 public List<TodoEntity> delete(final TodoEntity entity) {
		 ...
		 try {
			 ...
			 // (3) exception 발생시 id 와 exception을 로깅한다
			 log.error("error deleting entity ", entity.getId(), e);
			 
			 // (4) 컨트롤러로 exception 을 날린다. 데이터 베이스 내부 로직을 캡슐화 하기 위해 e 를 리턴하지 않고 새 exception 오브젝트를 리턴한다
